...
class DBEvaluator:
    def __init__(self, args):
        ...
        self.doctors = [
            ...
            ("GPT-4", "D:\\HuaweiMoveData\\Users\\王翔\\Desktop\\AI_Hospital-main\\dialog_history_ds_max_v3_base_eye.jsonl"), 
    ...
    def parse_diagnosis(self):
        ...
        if os.path.exists(self.eval_save_filepath):
            with open(self.eval_save_filepath, "r", encoding='utf-8') as inputfile:
        ...
        for doctor_name in self.doctor_names:
            ...
            for patient_id in tqdm(self.patient_ids):
                ...
                if doctor_diagnosis is None or '#诊断结果#' not in doctor_diagnosis['diagnosis'] or '#诊断依据#' not in doctor_diagnosis['diagnosis']:
                    continue
                # filtered_log_path = "D:\\HuaweiMoveData\\Users\\王翔\\Desktop\\AI_Hospital-main\\filtered_cases_log.txt"

                # if processed_doctor_name_w_patient_id.get((doctor_name, patient_id)) is not None:
                #     with open(filtered_log_path, "a", encoding="utf-8", newline="\n") as f:
                #         f.write(f"{doctor_name}\t{patient_id}\tSKIP_ALREADY_PROCESSED\n")
                #     continue
                ...
                #     with open(filtered_log_path, "a", encoding="utf-8", newline="\n") as f:
                #         f.write(f"{doctor_name}\t{patient_id}\tSKIP_DOCTOR_DIAGNOSIS_NONE\n")
                #     continue

                # diag_text = doctor_diagnosis.get("diagnosis", "")
                # if '#诊断结果#' not in diag_text or '#诊断依据#' not in diag_text:
                #     with open(filtered_log_path, "a", encoding="utf-8", newline="\n") as f:
                #         f.write(f"{doctor_name}\t{patient_id}\tSKIP_BAD_FORMAT\n")
                #     continue
    ...
    def execute_match(self, data):
        ...
        with open(self.eval_save_filepath, 'a', encoding='utf-8', newline="\n") as outfile:
...
def get_args():
    ...
    parser.add_argument("--model_name", type=str, default="openai/gpt-4o-mini", help="model name of openai to act as reference for evaluation")
    parser.add_argument("--database", type=str, default="D:\\HuaweiMoveData\\Users\\王翔\\Desktop\\AI_Hospital-main\\src\\evaluate\\ICD-10.xls", help="database")
    parser.add_argument("--eval_save_filepath", type=str, default="D:\\HuaweiMoveData\\Users\\王翔\\Desktop\\AI_Hospital-main\\src\\db_eval\\dialog_history_ds_max_v3_base_eye_db.jsonl", help="save path for evaluation results")
    parser.add_argument("--reference_diagnosis_filepath", type=str, default="D:\\HuaweiMoveData\\Users\\王翔\\Desktop\\AI_Hospital-main\\src\\data\\patients_subset.json", help="reference diagnosis filepath") 
    parser.add_argument("--doctors", type=str, nargs="+", default=["GPT-4"], help="doctor results for evaluation")
