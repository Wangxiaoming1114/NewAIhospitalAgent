...
class Consultation:
    def __init__(self, args):
        ...
        if args.RL_type == 'All' or args.RL_type == 'diagnosis':
            disease_list = self.get_action_list('/home/wangsihan/AI_Hospital-main/src/data/diagnosis_action.json')

            action_list = ['明确辅助诊断目的', '结合病史','分析异常值','综合其他检查','排除干扰因素','参考诊断标准','提出初步诊断','鉴别诊断', '制定进一步检查计划', '明确诊断', '制定治疗计划', '给出方案']
            self.medical_director_summary_query = \
                "现在你需要根据病人的症状和辅助检查结果，进行诊断和治疗。您能分别总结一下病人的症状和辅助检查的结果，然后给出您的诊断结果、诊断依据和治疗方案吗？" + \
                "按照下面的格式给出。\n\n" + \
                f"你可以采取下列的行为: {action_list}\n" + \
                f"在诊断时，有这些系统可以回顾: {disease_list}. \n" +\
                "请全面的分析，病情，并给出最后的回复。按照下面的格式给出。\n\n" + \
                "#症状#\n(1)xx\n(2)xx\n\n" + \
                "#辅助检查#\n(1)xx\n(2)xx\n\n" + \
                "#诊断结果#\nxx\n\n" + \
                "#诊断依据#\n(1)xx\n(2)xx\n\n" + \
            ...
            self.medical_director_summary_query = \
                ...
                "#诊断依据#\n(1)xx\n(2)xx\n\n" + \
                "#治疗方案#\n(1)xx\n(2)xx" 
        print(self.medical_director_summary_query)
    ...
    def add_parser_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument("--RL_type", default='All', help="options: All, inquiry,examination, diagnosis")

    def get_action_list(self, path):
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        categories = list(data.keys())
        return categories+['给出结论']
    ...
    def _diagnosis(self, patient):
        ...
        for turn in range(20):#self.max_conversation_turn):
            ...
            # elif speak_to == '门诊医生':
            #     print(speak_to, '开始')
            #     doctor_response = self.doctor.analysis_test(patient_response, patient.id)
            elif speak_to == "检查员":
