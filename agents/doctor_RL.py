...
class Doctor(Agent): #问诊时不需要完成所有的行为
    def __init__(self, engine=None, doctor_info=None, RL_type='All', name="A"):
        if doctor_info is None:
            inquiry_action_list = self.get_action_list(type='inquiry')
            test_action_list = self.get_action_list(type='test')
            # self.system_message = \
            #     "现在你们是个由多个医生组成的医疗团队，你们需要相互配合实现目标，目标是给病人提供正确有效的诊断和治疗。\n"+ \
            #     "你是一个专业且耐心的医生，你需要向病人问诊，你的个人目标是向病人了解全面且有效的病情并给病人选择合适的辅助检查:\n" + \
            #     f"你主要需要做两件事情，1. 问诊，2. 选择辅助检查，在问诊阶段，你可以采取下列的行为: {inquiry_action_list}. \n" +\
            #     f"在选择辅助检查阶段，你可以采取下列的行为: {test_action_list}. \n" +\
            #     "基于观察，你需要选择最好的行为去实现你的目标。\n"+\
            #     "请注意，请你回复行为的选择，并且如何行动。按照以下格式：行为，<具体>: 描述，比如：主诉，<具体>: “症状持续了多长时间？”\n" +\
            #     "(1) 在信息不充分的情况下，不要过早结束对话。\n" + \
            #     "(2) 每次只提一个问题，尽量简短。\n" +\
            #     "(3) 问诊请尽可能的详细，不要遗漏重要细节，并在必要的时候结束问诊即可 \n"   + \
            #     "(4) 请务必在结束问诊之后，才能开启选择辅助检查阶段。" + \
            #     "(5) 辅助检查的阶段，根据患者的病情选择最合适的行为，选择尽可能详尽的检查，按照一下格式：比如：实验室检查，<具体>: 粪便检查 隐血试验\n"   + \
            #     "(6) 病人会将辅助检查告知你，请你进行初步分析，如果有追加的检查，也一定要追问检查结果。\n"   + \
            #     "(6) 请务必在看到检查结果和追加检查结果后，才给出诊断。"
            if RL_type == 'All':
                self.system_message = \
                    "现在你们是个由多个医生组成的医疗团队，你们需要相互配合实现目标，目标是给病人提供正确有效的诊断和治疗。\n"+ \
                    "你是一个专业且耐心的医生，你需要向病人问诊，你的个人目标是向病人了解全面且有效的病情并给病人选择合适的辅助检查:\n" + \
                ...
                self.system_message = \
                    ...
                    f"(7) 在询问病情的时候，你的目标是全面且有效的病情，你可以采取下列的行为: {inquiry_action_list}。请基于观察，选择最好的行为去实现你的目标。按照以下格式：行为，<具体>: 描述，比如：主诉，<具体>: “症状持续了多长时间？"
            elif RL_type == 'examination':
                self.system_message = \
                    "你是一个专业且耐心的医生，下面会有患者向你咨询病情。你需要：\n" + \
                    "(1) 在信息不充分的情况下，不要过早作出诊断。\n" + \
                    "(2) 多次、主动地向患者提问来获取充足的信息。\n" + \
                    ...
                    "(5) 最后根据患者的身体状况和检查结果，给出诊断结果、对应的诊断依据和治疗方案。\n" + \
                    "(6) 诊断结果需要准确到具体疾病，治疗方案中不要包含检查。\n" + \
                    f"(7) 在要求患者进行检查的时候，你的目标是给病人选择合适的辅助检查，你可以采取下列的行为: {test_action_list}。请基于观察，选择最好的行为去实现你的目标。按照一下格式：比如：实验室检查，<具体>: 粪便检查 隐血试验"
            else:
                self.system_message = \
                "你是一个专业且耐心的医生，下面会有患者向你咨询病情。你需要：\n" + \
                "(1) 在信息不充分的情况下，不要过早作出诊断。\n" + \
        ...
        self.long_memories = defaultdict(default_diagnosis_factory) 

    def get_action_list(self, type='test'):
        if type == 'test':
            with open("/home/wangsihan/AI_Hospital-main/src/data/test_action.json", "r", encoding="utf-8") as f:
                data = json.load(f)
            categories = list(data.keys())
            return categories+['查体（体格检查）', '结束']
        if type == 'inquiry':
            with open("/home/wangsihan/AI_Hospital-main/src/data/inquiry_action.json", "r", encoding="utf-8") as f:
                data = json.load(f)
            categories = list(data.keys())
            return categories+['手术史', '结束问诊，开始选择辅助检查']
    ...
    def analysis_test(self, patient, patient_response):
        # load the symptom and examination from the host
        long_memories = self.long_memories[patient.id]
        # self.load_diagnosis(
        #     diagnosis=symptom_and_examination, 
        #     patient_id=patient.id
        # )
        action_list = self.get_action_list(type='test')
        # revise the diagnosis
        # build the system message
        system_message = "现在你们是个由多个医生组成的医疗团队，你们需要相互配合实现目标，目标是给病人提供正确有效的诊断和治疗。\n"+ \
            "你是一个专业的医生，现在你的同事已经对患者进行了问诊，你的任务是根据问诊结果建议病人开展有效的辅助检查。\n" + \
            f"你可以采取下列的行为: {action_list}. \n" +\
            "基于观察，你需要选择最好的行为去实现你的目标。\n"+\
            "请注意，请你回复行为的选择，并且如何行动。按照以下格式：行为，<具体>: 描述，比如：实验室检查，<具体>: 粪便检查 隐血试验 \n" +\
            "一次选择一个行动，在你觉得合适时结束，结束时告诉患者找检查员做检查。"
        # build the content
        content = "#问诊总结#\n{}, #病人的话#\n{} ".format(
            long_memories['问诊总结'], patient_response
        )
    ...
    def speak(self, content, patient_id, save_to_memory=True):
        ...
        if '结束问诊' in responese:
            self.long_memories[patient_id]['问诊总结'] = responese
...
class GPTDoctor(Doctor):
    def __init__(self, args=None, doctor_info=None, name="A"):
        ...
        super(GPTDoctor, self).__init__(engine, doctor_info, RL_type=args.RL_type, name=name)
...
class ChatGLMDoctor(Doctor):
    def __init__(self, args=None, doctor_info=None, name="A"):
        ...
        super(ChatGLMDoctor, self).__init__(engine, doctor_info, RL_type=args.RL_type, name=name)
...
class WenXinDoctor(Doctor):
    def __init__(self, args=None, doctor_info=None, name="A"):
        ...
        super(WenXinDoctor, self).__init__(engine, doctor_info,RL_type=args.RL_type, name=name)
...
class QwenDoctor(Doctor):
    def __init__(self, args=None, doctor_info=None, name="A"):
        ...
        super(QwenDoctor, self).__init__(engine, doctor_info,RL_type=args.RL_type, name=name)
    ...
    def add_parser_args(parser):
        ...
        parser.add_argument(
            ...
            choices=["deepseek-v3", "qwen-max", "qwen-max-1201", "qwen-plus-gamma", "qwen-plus", "qwen-turbo", "baichuan2-7b-chat-v1", "baichuan2-13b-chat-v1"], default="qwen-max")
