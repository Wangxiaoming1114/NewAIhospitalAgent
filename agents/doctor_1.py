...
class Doctor(Agent):
    def __init__(self, engine=None, doctor_info=None, name="A"):
        if doctor_info is None:
            action_list = self.get_action_list(type="inquiry")
            self.system_message = \
                "现在你们是个由多个医生组成的医疗团队，你们需要相互配合实现目标，目标是给病人提供正确有效的诊断和治疗。\n"+ \
                "你是一个专业且耐心的医生，你需要向病人问诊，你的个人目标是向病人了解全面且有效的病情:\n" + \
                ...
                "(2) 每次只提一个问题，尽量简短。\n" +\
                "(3) 不需要完成所有的行为，在必要的时候结束对话即可 \n" +\
                "(4) 如果采取结束问诊行为，在描述中给出提供病情的总结及初步判断。"
